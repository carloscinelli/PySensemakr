...
`PySensemakr` is a Python package that implements this full suite of sensitivity analysis tools
for linear regression models. The package allows researchers to
...
is also available. While these implementations serve the R and Stata communities well,
Python has lacked an equivalent tool, despite being one of the most widely used
languages for data analysis across the social sciences, biomedical research, and industry.
`PySensemakr` fills this gap, bringing the full `sensemakr` methodology to the Python ecosystem
and lowering the barrier for researchers who wish to incorporate formal
...
\autoref{fig:contour} illustrates the contour plot output, in which the estimate and the $t$-value of the treatment effect are displayed as functions of the hypothetical confounding strength. \autoref{fig:extreme} shows the corresponding extreme scenario plot.
