...
closely mirrors the R version. Rather than contributing to an existing Python package,
we chose to build a standalone implementation because no existing Python package provides
these sensitivity analysis tools, and the methodology requires a tightly integrated set
of functions---from sensitivity statistics to bias-adjusted inference to specialized
visualizations---that would not fit naturally as an extension of a general-purpose
regression or plotting library.
...
models, which allows `PySensemakr` to integrate seamlessly into existing regression
...
users familiar with the R `sensemakr` package to transition to Python. All numerical
results are validated against the R implementation to ensure correctness.

The following code illustrates a typical workflow using the example dataset of
@hazlett2020angry:
...
Generative AI tools (Claude, Anthropic) were used to assist with code drafting,
documentation, and paper preparation. All AI-generated content was reviewed, verified,
and edited by the authors. Numerical outputs were validated against the R `sensemakr`
package to ensure correctness.
